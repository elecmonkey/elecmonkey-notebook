...
import { buildLR0Table, LRTable, LR0Item, Action } from './utils/lr0-algo'
...
const lr0Result = ref<LRTable | null>(null)
const errorMsg = ref('')
...
function analyze() {
  ...
  try {
    ...
    lr0Result.value = buildLR0Table(g)
...
const conflictCount = computed(() => {
  if (!lr0Result.value) return 0
  let count = 0
  lr0Result.value.action.forEach(row => {
    row.forEach(actions => {
...
<template>
  <div class="lr0-analyzer-container">
    <div class="input-section">
      <div class="header">
        <span class="title">Grammar Input (LR(0))</span>
    ...
    <div v-if="lr0Result" class="results-section">
      
      <div class="status-bar">
        <div class="stat-item">
          States: <strong>{{ lr0Result.states.length }}</strong>
        </div>
      ...
      <div v-if="activeTab === 'table'" class="table-container">
        <table class="lr-table">
          ...
          <tbody>
            <tr v-for="state in lr0Result.states" :key="state.id">
              <td class="state-id-cell">{{ state.id }}</td>
              <!-- ACTION -->
              <td v-for="t in terminalsList" :key="t" 
                  :class="{ 'conflict-cell': (lr0Result.action.get(state.id)?.get(t)?.length || 0) > 1 }">
                {{ formatAction(lr0Result.action.get(state.id)?.get(t)) }}
              </td>
              <!-- GOTO -->
              <td v-for="nt in nonTerminalsList" :key="nt">
                {{ lr0Result.goto.get(state.id)?.get(nt) ?? '' }}
              </td>
            </tr>
          </tbody>
        </table>
