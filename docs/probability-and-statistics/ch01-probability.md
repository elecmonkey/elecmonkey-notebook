...
随机试验的所有可能结果组成的集合称为**样本空间**，记作$S$或$\Omega$。
...
1. **包含关系**：
   事件$A$发生必然导致事件$B$发生，记作$A \subseteq B$。
2. **相等关系**：
   事件$A$和$B$同时发生或不发生，记作$A = B$。
3. **互斥关系**：
   事件$A$和$B$不能同时发生，记作$A \cap B = \emptyset$。
4. **对立关系**：
...
3. **可加性**：若事件$A_1, A_2, \dots, A_n$互不相容，则：
   $$
   P\left(\bigcup_{i=1}^{n} A_i\right) = \sum_{i=1}^{n} P(A_i)
   $$
...
设$B_1, B_2, \dots, B_n$是一组**互斥且完备**的事件，即满足以下条件：

1. **互斥性**：$B_i \cap B_j = \emptyset$ 对于所有$i \neq j$。
2. **完备性**：$\bigcup_{i=1}^n B_i = \Omega$，其中$\Omega$是样本空间。
...
1. 若$A$和$B$独立，则$A$和$\overline{B}$、$\overline{A}$和$B$、$\overline{A}$和$\overline{B}$也独立。
