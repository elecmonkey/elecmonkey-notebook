...
### 3.4.1 构造可靠传输协议 (rdt 1.0 - 3.0)
为了应对信道可能出现的比特差错、丢包、乱序等问题，协议的设计需要逐步演进：

1.  **rdt 1.0：完全可靠信道**
    *   **假设**：底层信道完全可靠（不丢包、不错位）。
    *   **机制**：发送方直接发送，接收方直接接收，无需任何额外控制。

2.  **rdt 2.0：具有比特差错的信道**
    *   **假设**：信道可能翻转比特（0 变 1），但不会丢包。
    *   **差错检测 (Error Detection)**：利用**校验和 (Checksum)** 检测数据在传输过程中是否损坏。
    *   **接收者反馈 (Feedback)**：
        *   **ACK (肯定确认)**：接收方告诉发送方“收到了，没问题”。
        *   **NAK (否定确认)**：接收方告诉发送方“收到了，但有错”。
    *   **重传 (Retransmission)**：当收到 NAK 时，发送方重传该报文。这种基于重传的机制称为 **ARQ (自动重传请求)**。

3.  **rdt 2.1 / 2.2：处理 ACK/NAK 出错**
    *   **问题**：如果 ACK/NAK 包本身在传输中坏了，发送方不知道该发新的还是重传旧的。
    *   **序号 (Sequence Number)**：
        *   给每个分组加上序号（如 0 或 1）。
        *   若发送方收到模糊不清的 ACK，它就重传当前分组。
        *   接收方通过检查**序号**来判断这是“新的分组”还是“重复的旧分组”（如果是重复的则丢弃并再次发送 ACK）。

4.  **rdt 3.0：具有丢包的信道**
    *   **假设**：信道既可能错位，也可能**丢包**（数据或 ACK 丢失）。
    *   **定时器 (Timer)**：
        *   发送方每发送一个分组就启动一个**倒计时定时器**。
        *   如果在时间内收到 ACK，则终止定时器。
        *   如果**超时 (Timeout)** 仍未收到 ACK，发送方认为丢包，触发**重传**。
    *   这就是著名的**停等协议 (Stop-and-Wait)**：发一个，等一个；没等到就重发。
    *   **缺点**：**信道利用率 (Utilization)** 极低。在长肥管道（高带宽延迟积）中，大部分时间都在傻等。

### 3.4.2 流水线协议 (Pipelining)
为了解决停等协议效率低的问题，引入了**流水线**技术：允许发送方在未收到确认的情况下，连续发送多个分组。
这带来了三个变化：
1.  **必须增加序号范围**：不能只用 0 和 1 了，需要更大的序号空间。
2.  **发送方和接收方需要缓存**：用于暂存已发送未确认的分组，或乱序到达的分组。
3.  **需要更复杂的错误恢复机制**：即 GBN 和 SR。

### 3.4.3 滑动窗口 (Sliding Window)
滑动窗口是实现流水线协议的核心机制。
*   **发送窗口**：允许发送但未被确认的序号集合。窗口大小 `N` 决定了流水线的能力。
    *   `Base`：最早的未确认序号。
    *   `NextSeqNum`：下一个待发送序号。
    *   窗口范围：`[Base, Base + N - 1]`。
*   随着 `ACK` 的到来，`Base` 向前移动（窗口右滑），允许发送新的分组。

### 3.4.4 回退 N 步 (Go-Back-N, GBN)
GBN 是一种“一损俱损”的策略，侧重于简化接收方逻辑。

*   **发送方行为**：
    1.  **发送**：只要窗口未满，就可发送。
    2.  **累计确认 (Cumulative ACK)**：收到 `ACK(n)` 表示序号 `n` **及之前**的所有分组都已正确接收。
    3.  **单一定时器**：只为**最早的未确认分组 (Base)** 维护一个定时器。
    4.  **超时重传**：一旦超时，重传窗口内**所有**已发送但未确认的分组（回退 N 步）。

*   **接收方行为**：
    1.  **只接收顺序分组**：维护 `expectedseqnum`。只有收到的分组序号正好等于 `expectedseqnum` 才接收并交付。
    2.  **丢弃乱序**：其他任何乱序分组（不管是否正确）一律**丢弃**。
    3.  **重复确认**：每丢弃一个乱序包，就重发一次当前即使确认的 ACK（如一直发 `ACK(expectedseqnum-1)`）。

*   **特点**：
    *   优点：接收方简单，不需要缓存乱序包。
    *   缺点：一旦出错，重传一大批包，浪费带宽。

### 3.4.5 选择重传 (Selective Repeat, SR)
SR 是一种“精细化管理”的策略，侧重于效率。

*   **发送方行为**：
    1.  **独立确认**：每个分组都需要单独的 ACK。
    2.  **多定时器**：为**每个**发出的分组单独维护一个定时器。
    3.  **超时重传**：哪个分组超时，就只重传哪个分组。

*   **接收方行为**：
    1.  **缓存乱序**：收到乱序分组（在接收窗口内），先**缓存**下来，不丢弃。
    2.  **按序交付**：当缺失的分组终于补齐时，将缓存的一批连续分组一起交付给上层。
    3.  **独立 ACK**：收到谁就确认谁（如 `ACK(n)` 仅表示收到 `n`）。

*   **特点**：
    *   优点：链路效率高，仅重传出错的包。
    *   缺点：实现复杂（逻辑复杂，需要更多缓存及定时器）。
    *   **窗口限制**：接收窗口大小不能超过序号空间的一半，否则会产生歧义（新旧窗口重叠）。
...
| **定时器** | 单一 (最早未确认) | 多个 (每个分组一个) |
| **接收方缓存** | 无 (丢弃乱序) | 有 (缓存乱序) |
| **重传策略** | 重传窗口内所有分组 | 仅重传超时的分组 |
| **适用场景** | 出错率低、接收端简单的环境 | 链路昂贵、出错率较高的环境 |
